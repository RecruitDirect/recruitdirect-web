...
function signUpWithPassword() {
    return authStore.signUp(fname.value, email.value, password.value, role.value).catch((error) => console.log(error));
}

function signUpUserWithGoogle() {
    return authStore.signInGoogle(role.value).catch((error) => console.log(error));
}
...
<template>
    <div>
        <v-label class="text-subtitle-1 font-weight-medium pb-2 text-lightText">Role</v-label>
        <v-radio-group v-model="role" direction="horizontal" @update:modelValue="setRole()">
            <v-radio label="Recruiter/Referrer" value="recruiter"></v-radio>
            <v-radio label="Hiring company" value="hiringclient"></v-radio>
            <v-divider :thickness="2" class="border-opacity-100 mt-3" color="black"></v-divider>
        </v-radio-group>
    </div>
    <div v-if="role != ''">
